...
class TrieNode():
    __slots__ = 'Index', 'Layer', 'End', 'Char', 'Results', 'm_values', 'Failure', 'Parent'
    ...
    def SetResults(self, index)-> None:
        if (self.End == False):
            self.End = True
...
class TrieNode2():
    __slots__ = 'End', 'Results', 'm_values', 'minflag', 'maxflag'

    def __init__(self):
        self.End: bool = False
        self.Results: list = []
        self.m_values: dict = {}
        self.minflag: int = 0xffff
        self.maxflag: int = 0

    def Add(self, c: str, node3)-> None:
        if (self.minflag > c):
            self.minflag = c
        if (self.maxflag < c):
            self.maxflag = c
    ...
    def HasKey(self, c)-> bool:
        return c in self.m_values

    def TryGetValue(self, c)-> str:
...
class StringSearch():
    ...
    def SetKeywords(self, keywords: list[str]):
        ...
        i = 0
        while i < len(self._keywords):  # for (i = 0; i < _keywords.length; i++)
            p = self._keywords[i]
            nd = root
            j = 0
            while j < len(p):  # for (j = 0; j < p.length; j++)
                nd = nd.Add(ord(p[j]))
                if (nd.Layer == 0):
                    nd.Layer = j + 1
